...
# scenario <name> <tracked-exports: 0|1>
#
...
for n in $(seq 1 "$ISSUES"); do
  bd create --title="stress issue $n" --parent="$EPIC" \
    --description="Rewrite hot.txt, commit, close." --silent >/dev/null
done
echo "  $EPIC with $ISSUES children"
...
if [ "$done_n" = "$ISSUES" ]; then pass "all $ISSUES issues done"; else
  fail "done $done_n of $ISSUES"
fi

if [ "$status" = "done" ]; then pass "the run finished"; else fail "run status $status"; fi

# Every worker's line is in the contested file, so no resolution dropped a side.
missing=""
for id in $(python3 -c "
...
if [ -z "$missing" ]; then pass "every branch's work is in the tree"; else
  fail "lost:$missing"
