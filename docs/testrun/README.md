...
held nothing but a stub `main` and a `go.mod`. Four beads epics, eleven issues,
a dependency graph shaped to produce several waves and genuine parallelism:
...
kv-jp9  the usage listing          kv-169  command dispatch
  kv-jp9.1  mark the writers    ┐     kv-169.1  suggest the nearest  ┐
  kv-jp9.2  wrap at 80 columns  ┘     kv-169.2  -h and --help        ┘
```

The last two pairs are the point of the last two epics: each pair rewrites one
function, deliberately, in one wave, so the barrier meets a real conflict.
...
0
$ kv gte foo
kv: unknown command "gte"; did you mean "get"?
```

Every one of those listings is the merged result of two branches that both
rewrote the function behind it. A model resolved each conflict; see drains 3
and 4.

## The runs

| | drain 1 — `kv-ctf` | drain 2 — `kv-555` | drain 3 — `kv-jp9` | drain 4 — `kv-169` |
|---|---|---|---|---|
| scope | 3 issues | 4 issues, over three legs | 2 issues | 2 issues |
| autonomy | `auto` | `wave` — paused at every barrier | `auto` | `auto` |
| waves | 2 | 3 + 1 after an unpark | 1 | 1 |
| what it exercised | the happy path end to end | a question, a kill, a park, an unpark, a resume | two workers on one function, a real conflict | the same again, against the fixed barrier |
| pull request | opened | refused — an issue was parked | opened | opened |
| cost / turns | $3.0383 / 61 | $5.8739 / 129 | $3.1958 / 75 | $2.7466 / 72 |
...
| # | goal | verdict |
|---|---|---|
| 1 | A drain builds working software from beads issues alone | met — the program above, 11 issues, no human edit to any of it |
| 2 | Waves respect the dependency graph and run in parallel where it allows | met — drain 1 ran waves of 1 then 2, drain 2 of 1, 2, 1, drains 3 and 4 of 2 |
...
| 4 | A conflict between two correct changes is resolved by a model, not by hand | met twice — drain 3 on `internal/cli/cli.go` and drain 4 on its test, both resolved and gated green |
...
| 6 | A finished, clean run pushes and opens a pull request | met — drains 1, 3 and 4, through the recording `gh`; bodies beside this file |
...
| 11b | The barrier is legible while it works, live | met — drain 4, `live-19-barrier-at-work`: "wave 1 integrating: merging 2 branches" with the integrator's tool call on the row whose branch it was resolving |
| 12 | The run's own cost and turn totals match the raw transcripts | met — drains 1, 3 and 4 reconcile exactly ($3.0383/61, $3.1958/75, $2.7466/72); drain 2 does not, and that is `beads-auto-imp-xzx` below |
| 13 | Every defect found is filed in beads and fixed immediately | met — ten issues, below |
...
`wne` was proven fixed the same way it was found: by running. Drain 4 is drain
3's shape again — two issues, one function, one wave — against the fixed binary.
Compare `live/bug-invisible-integrator` with `live/live-19-barrier-at-work`:
same moment, same kind of conflict, and the second one says what is happening.
When the barrier finished, the row that had been showing the integrator's tool
calls settled to `merged; a model resolved 1 conflict`.
